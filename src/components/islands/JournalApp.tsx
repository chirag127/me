...
export default function JournalApp() {
  ...
  useEffect(() => {
    ...
    if (user) {
      (async () => {
        unsubscribeEntries = await subscribeToJournalEntries(user.uid, (data) => {
          setEntries(data);
        });
      })();
    } else {
    ...
    return () => { if (unsubscribeEntries) unsubscribeEntries(); };
  }, [user]);
  ...
  if (!user) {
    return (
      <div className="glass-strong p-12 text-center rounded-2xl flex flex-col items-center justify-center">
        ...
        <h2 className="text-2xl font-bold text-white mb-3">Sign in to Journal</h2>
        <p className="text-white/50 mb-8 max-w-sm mx-auto">Your journal entries will be safely synced and securely linked to your email address in Firestore.</p>
        <button 
    ...
    <>
      <div className="glass-strong p-8 mb-10">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-white/20">{input.length} characters</span>
          <button 
            onClick={handleSave} 
            disabled={!input.trim()}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs text-white/30">{entries.length > 0 ? `${entries.length} entries` : ""}</span>
      </div>
      
      <div className="space-y-4">
        {entries.length === 0 ? (
          <div className="text-center text-white/30 text-sm py-12">
            <svg className="h-12 w-12 mx-auto mb-3 text-white/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <div key={entry.id} className="glass p-6 group">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-amber-400/70">{dateStr}</span>
                ...
                <p className="text-sm text-white/80 whitespace-pre-wrap leading-relaxed">{entry.text}</p>
              </div>
            );
          })
        )}
