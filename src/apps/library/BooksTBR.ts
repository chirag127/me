...
export default async function BooksTBR(c: HTMLElement): Promise<void> {
  ...
  try {
    ...
    const totalCount = user?.reading_log_counts?.want_to_read || readingLog.total_results;
    ...
    c.innerHTML = `
      <div class="page animate-fade-in">
        ...
        <div class="glass-panel empty-state">
          ...
          <p>There was an error fetching the reading queue. Please try again later.</p>
