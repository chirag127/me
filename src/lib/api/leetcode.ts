...
async function fetchLeetcode<T>(query: string, variables: Record<string, any>): Promise<T | null> {
  try {
...
export async function fetchLeetCodeStats() {
  const data = await fetchLeetcode<any>(query, { username: CONFIG.user.leetcode });
  ...
  return {
    ...
    acceptanceRate: Math.round(((allInfo?.count || 0) / Math.max((allInfo?.submissions || 1), 1)) * 100),
  };
}
