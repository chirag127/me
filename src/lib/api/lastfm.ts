...
import type { LastFmAlbum, LastFmArtist, LastFmTrack } from './types';
...
export async function fetchLastFmStats(): Promise<{
  playcount: number;
  registered: string;
} | null> {
  const data = await fetchJson<any>(
    `${getBaseUrl()}&method=user.getinfo`,
    undefined,
    'LastFM',
  );
...
export async function fetchLastFmTopArtists(
  ...
  return data.topartists.artist.map((a: any) => ({
    ...
    url: a.url,
    imageUrl:
      a.image?.find((img: any) => img.size === 'extralarge')?.['#text'] || null,
  }));
}
...
export async function fetchLastFmTopTracks(limit = 10): Promise<LastFmTrack[]> {
  ...
  return data.toptracks.track.map((t: any) => ({
    ...
    url: t.url,
    imageUrl:
      t.image?.find((img: any) => img.size === 'extralarge')?.['#text'] || null,
  }));
}
