...
export async function fetchOpenLibraryBooks(status: 'want-to-read' | 'currently-reading' | 'already-read'): Promise<OpenLibraryBook[]> {
...
export async function fetchBookDetails(key: string): Promise<string[]> {
  ...
  const data = await fetchJson<any>(`${OL_API_URL}${key}.json`, undefined, 'OpenLibrary');
