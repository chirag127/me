...
for (const [key, value] of entries) {
  if (key === "GH_TOKEN" || key === "GITHUB_TOKEN" || key === "GITHUB_PERSONAL_ACCESS_TOKEN" || key === "GH_USERNAME") {
    continue;
  }
  ...
  if (result.status === 0) {
    ...
    const errorStr = result.error ? result.error.message : (result.stderr || "Timeout or unknown error");
