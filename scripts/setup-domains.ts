...
async function addDomainToCloudflarePages(domain: string) {
  ...
  const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/pages/projects/${PROJECT_NAME}/domains`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    ...
    console.error(`[Cloudflare] Failed to add ${domain} (${res.status} ${res.statusText}):`, text);
    return;
  }
  ...
  if (data.success) {
    ...
    if (data.errors && data.errors.some((e: any) => e.message.includes('already exists'))) {
      ...
      console.error(`[Cloudflare] Failed to add ${domain}:`, JSON.stringify(data.errors));
    }
  }
}
...
async function getSpaceshipRecords(rootDomain: string) {
  ...
  if (!res.ok) {
    console.error(`[Spaceship] Failed to fetch records for ${rootDomain}: ${res.status} ${res.statusText}`);
    return [];
  }
  const data = await res.json() as any;
  return data.items || [];
...
async function updateSpaceshipRecords(rootDomain: string, records: any[]) {
  ...
  if (!res.ok) {
    console.error(`[Spaceship] Failed to update records for ${rootDomain}: ${res.status} ${res.statusText}`);
    const text = await res.text();
    console.error(text);
    return false;
  }
  console.log(`[Spaceship] Successfully updated DNS records for ${rootDomain}.`);
  return true;
}
  ...
  for (const [rootDomain, subs] of rootDomains.entries()) {
    ...
    for (const { sub, full } of subs) {
      ...
      for (let i = records.length - 1; i >= 0; i--) {
        const r = records[i];
        if (r.name === sub && ['A', 'AAAA', 'CNAME', 'ALIAS'].includes(r.type)) {
          records.splice(i, 1);
        }
      }
      ...
      if (sub === '@') {
        ...
        modified = true;
        console.log(`[Spaceship] Added ALIAS for ${full} -> ${TARGET_PAGES_DOMAIN}`);
      } else {
        ...
        modified = true;
        console.log(`[Spaceship] Added CNAME for ${full} -> ${TARGET_PAGES_DOMAIN}`);
      }
    }
